...
export class Agent implements IAgent {
  ...
  async rejiggerConfig(newConfig: AgentConfig): Promise<void> {
    if (
      this.config.model_vendor !== newConfig.model_vendor ||
  ...
  async performInference(
    ...
    for await (const response of this.client.sendMessage(
      this.messages,
      userMessage,
      RAGContext
    )) {
      if (
        ...
        (this.config.json_stdout_override ||
          this.config?.response_format !== 'json_object')
      ) {
