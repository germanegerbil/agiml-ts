...
async function main() {
  ...
  const agent = await AgentFactory.createFromSettings(config as any);
  agent.addMiddleware(new AGIMLMiddleware(agimlPluginConfig));

  // Example 1: Simple text
  const response1 = await agent.performInference(
    'Hello Oscar, how is life? Do you have any questions about the AGIML training manual?'
  );
